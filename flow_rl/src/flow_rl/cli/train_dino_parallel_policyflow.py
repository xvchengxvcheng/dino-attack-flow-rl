"""Train version-isolated PolicyFlow across 16 Dino Unity Players."""
...
from flow_rl.algorithms.policyflow import PolicyFlowUpdater
from flow_rl.cli.collect_dino_ppo_demonstrations import _scan_unity_logs
...
from flow_rl.training.dino_batched_policy import DinoBatchedPolicyFlowDecider
from flow_rl.training.dino_parallel_policyflow_config import (
    DinoParallelPolicyFlowCheckpointManager,
...
from flow_rl.training.policyflow_trainer import update_versioned_policyflow
...
def _build_policy_and_state(config, protocol, device):
    ...
    bc = load_checkpoint(config.actor_initialization_checkpoint_path, map_location=device)
    policy.actor.load_state_dict(bc["model_state"], strict=True)
    policy.refresh_snapshot()
    normalizer = IdentityObservationNormalizer(
        protocol.observation_shapes,
        protocol_manifest_sha256=protocol.manifest_sha256,
    )
    normalizer.load_state_dict(bc["normalizer_state"])
    action_generator = torch.Generator(device=device).manual_seed(config.seed + 7_000)
    update_generator = torch.Generator(device=device).manual_seed(config.seed + 8_000)
    updater = PolicyFlowUpdater(
        policy,
        actor_learning_rate=config.actor_learning_rate,
        critic_learning_rate=config.critic_learning_rate,
        clip_range=config.clip_range,
        gaussian_entropy_coefficient=config.gaussian_entropy_coefficient,
        brownian_coefficient=config.brownian_coefficient,
...
def run_training(config: DinoParallelPolicyFlowConfig) -> dict[str, object]:
    config.validate()
    ...
    manager = DinoParallelPolicyFlowCheckpointManager(
        config=config,
        protocol=protocol,
        policy=policy,
        updater=updater,
    ...
    if config.resume_checkpoint is not None:
        resume = manager.restore(config.resume_checkpoint)
    ...
    decide_batch = DinoBatchedPolicyFlowDecider(
        policy=policy,
        device=device,
        action_size=protocol.action_size,
        velocity_nfe=config.velocity_nfe,
        generator=action_generator,
    )

    def decide(environment_id, generation, policy_version, step, adapter):
        request = DinoParallelDecisionRequest(
            environment_id, generation, policy_version, step, adapter
        )
    ...
    def bootstrap(
        ...
        for target, observations in collector.bootstrap_observations.items():
            tensors = tuple(
                torch.as_tensor(item[None, ...], device=device)
                for item in observations
            )
            with torch.inference_mode():
                value = float(policy.critic(tensors).cpu().item())
            if not math.isfinite(value):
                raise RuntimeError("PolicyFlow produced non-finite bootstrap value")
            values[target] = value
        return values
    ...
    def checkpoint_update(update, absolute_steps: int) -> None:
        nonlocal optimizer_updates, next_checkpoint, peak_allocated, peak_reserved
        optimizer_updates += 1
        if device.type == "cuda":
        ...
        reporter.log_update(
            ...
            steps_per_second=(absolute_steps - resume.environment_steps)
            / max(current_seconds, 1e-9),
        )
    ...
    trainer = DinoParallelPPOTrainer(
        ...
        update_function=update_versioned_policyflow,
    )
    try:
    ...
    audit = audit_parameter_updates(initial_parameters, snapshot_named_parameters(policy))
    ...
    audit_md.write_text(render_parameter_update_audit_markdown(audit), encoding="utf-8")
    unity_audit = _scan_unity_logs(config.run_directory / "unity")
    result: dict[str, object] = {
        "status": "PASS" if unity_audit["critical_match_count"] == 0 else "FAIL",
    ...
    if result["status"] != "PASS":
        raise RuntimeError("PolicyFlow Unity log audit failed")
    return result
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = run_training(DinoParallelPolicyFlowConfig.from_yaml(args.config))
