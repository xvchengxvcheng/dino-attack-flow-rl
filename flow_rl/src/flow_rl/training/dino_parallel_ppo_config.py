...
class DinoParallelEnvironmentSpec:
    environment_id: int
    worker_id: int
...
class DinoParallelPPOConfig:
    ...
    @classmethod
    def from_yaml(cls, path: Path) -> "DinoParallelPPOConfig":
        source = Path(path).resolve()
        with source.open(encoding="utf-8") as handle:
            raw = yaml.safe_load(handle)
        if not isinstance(raw, Mapping):
            raise TypeError("Dino parallel PPO configuration must be a mapping")
        ...
        if not required.issubset(actual) or not actual.issubset(expected):
            raise ValueError(
                "Dino parallel PPO configuration keys mismatch; "
    ...
    def validate(self) -> None:
        ...
        if self.run_directory.is_dir() and any(self.run_directory.iterdir()):
            detail = (
                "resume requires a new empty run directory"
        ...
        if any(
            ...
            raise ValueError("step, batch, epoch, checkpoint and timeout counts must be positive")
        ...
        if self.num_envs not in (4, 8, 12, 16, 20, 24):
            raise ValueError(
        ...
        workers = [item.worker_id for item in self.environment_specs]
        seeds = [item.environment_seed for item in self.environment_specs]
        if len(set(workers)) != self.num_envs or len(set(seeds)) != self.num_envs:
            raise ValueError("parallel environments require unique worker IDs and seeds")

        if self.gamma != 0.995:
            raise ValueError("formal Dino PPO requires gamma=0.995")
        ...
        if self.device != "cuda":
            raise ValueError("formal Dino PPO requires device=cuda")
        if not isinstance(self.behavior_name, str) or not self.behavior_name:
        ...
        if (
            ...
            or not 1 <= self.inference_batch_size <= self.num_envs
        ):
            raise ValueError(
                "inference_batch_size must be between 1 and num_envs"
            )
        if not 0.0 <= float(self.inference_batch_wait_seconds) <= 0.1:
            raise ValueError(
                "inference_batch_wait_seconds must be within [0, 0.1]"
            )

        if self.encoder_type != "set_transformer":
            raise ValueError("formal Dino PPO requires the Set Transformer encoder")
        if self.hidden_sizes != (128, 128):
            raise ValueError("formal Dino PPO requires hidden_sizes=[128, 128]")
        if self.encoder_d_model != 48:
            raise ValueError("formal Dino PPO requires Set Transformer d_model=48")
        if self.encoder_heads != 4:
            raise ValueError("formal Dino PPO requires Set Transformer heads=4")
        if self.encoder_inducing_points != 8:
            raise ValueError("formal Dino PPO requires Set Transformer inducing_points=8")
        if self.encoder_layers != 1:
            raise ValueError("formal Dino PPO requires Set Transformer layers=1")
        if self.encoder_dropout != 0.0:
            raise ValueError("formal Dino PPO requires Set Transformer dropout=0.0")
        if self.encoder_output_size != 128:
            raise ValueError("formal Dino PPO requires Set Transformer output=128")
        ...
        if self.resume_checkpoint is not None:
            payload = load_checkpoint(self.resume_checkpoint, map_location="cpu")
            metadata = payload.get("metadata")
            if not isinstance(metadata, Mapping):
                raise ValueError("resume checkpoint metadata is missing")
            steps = metadata.get("environment_steps")
            if isinstance(steps, bool) or not isinstance(steps, int) or steps < 0:
                raise ValueError("resume checkpoint environment_steps is invalid")
            if self.total_environment_steps <= steps:
                raise ValueError(
                    "resume total_environment_steps must exceed checkpoint steps"
                )
            saved_protocol = metadata.get("protocol")
            if not isinstance(saved_protocol, Mapping) or saved_protocol.get(
                "protocol_version"
            ) != _PROTOCOL_VERSION:
                raise ValueError("resume checkpoint is not structured_set_v2")
...
class DinoParallelCheckpointManager:
    """Save and restore only clean, complete parallel PPO update boundaries."""
    ...
    def save(
        ...
        *,
        ...
        run_root = self.config.run_directory.resolve()
        ...
        if environment_steps <= 0 or policy_version <= 0 or optimizer_updates <= 0:
            raise ValueError("a checkpoint requires at least one complete PPO update")
    ...
    def restore(self, path: Path) -> DinoParallelResumeState:
        ...
        validate_checkpoint_compatibility(
            ...
            expected_encoder_metadata=encoder,
        )
        metadata = payload["metadata"]
        if metadata.get("algorithm") != "dino_parallel_ppo":
            raise ValueError("resume checkpoint algorithm mismatch")
        if metadata.get("rollout_boundary") is not True:
            raise ValueError("resume checkpoint is not a complete rollout boundary")
        if metadata.get("restart_boundary") is not True:
            raise ValueError("resume checkpoint must use restart-boundary semantics")
        if metadata.get("in_flight_decisions") != 0:
            raise ValueError("resume checkpoint contains in-flight decisions")
        if metadata.get("build_sha256") != _sha256(self.config.build_path):
            raise ValueError("resume checkpoint Unity build hash mismatch")
        saved_config = payload["config"]
        current_config = self.config.to_dict()
        mismatched = [
            key
            for key in self._STABLE_CONFIG_KEYS
            if saved_config.get(key) != current_config.get(key)
        ]
        if mismatched:
            raise ValueError(
                f"resume checkpoint configuration mismatch: {sorted(mismatched)}"
            )
        steps = _nonnegative_int(metadata, "environment_steps")
        version = _nonnegative_int(metadata, "policy_version")
        updates = _nonnegative_int(metadata, "optimizer_updates")
        if self.config.total_environment_steps <= steps:
            raise ValueError("resume target must exceed checkpoint steps")
        if version <= 0 or updates <= 0:
            raise ValueError("resume checkpoint has no completed PPO update")
        ...
        self.updater.optimizer.load_state_dict(optimizer_state)
        ...
        cuda_state = metadata["cuda_rng_state"]
    ...
    def _encoder_metadata(self) -> dict[str, Any]:
        encoder = self.policy.actor.encoder
        if not isinstance(encoder, StateEncoder):
            raise TypeError("Dino parallel PPO requires a structured actor encoder")
        return encoder.checkpoint_metadata()


def _nonnegative_int(metadata: Mapping[str, Any], name: str) -> int:
